const saveLocationFolderTitle = 'TabSaver';
const notFoundId = -1;
...
createBookmark = function(parentId, tab) {
    chrome.bookmarks.create(
        {'parentId': parentId, 'title': tab.title, 'url': tab.url}
...
createSaveLocation = function(parentId) {
    chrome.bookmarks.create({'parentId': parentId, 'title': 'Extension bookmarks'});
};
...
findIdForTitle = function(bookmarks, titleToFind) {
    ...
    findIdForTitleInternal = function(bookmarks, titleToFind) {
        for (var ii = 0; ii < bookmarks.length; ++ii) {
            var bookmark = bookmarks[ii];
            if (bookmark.title === titleToFind) {
                found = true;
                idOfTitle = bookmark.id;
                break;
            }
            if (bookmark.children) {
                findIdForTitleInternal(bookmark.children, titleToFind);
                if (found) {
                    break;
                }
            }
